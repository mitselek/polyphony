<script lang="ts">
	...
	async function copyInviteLink(link: string, memberName: string) {
		try {
			...
			toast.success(m.members_toast_invite_link_copied({ name: memberName }));
		} catch {
...
{#if filteredMembers.length === 0}
	<div class="py-12 text-center text-gray-500">
		{#if members.length === 0}
			<p>{m.members_no_members()}</p>
		{:else}
			<p>{m.members_no_search_results()}</p>
		{/if}
	</div>
{:else}
	<div class="space-y-4">
			...
			<Card variant="interactive" padding="lg" class="relative">
				...
				{#if member.id !== currentUserId && isOwner}
					<button
						...
						title={m.members_remove_title({ name: member.name })}
						aria-label={m.members_remove_aria({ name: member.name })}
					>
				...
				<div class="flex items-start justify-between">
					<div class="flex-1">
						<div class="flex items-center gap-3">
							...
							{#if member.id === currentUserId}
								<span
									...
									title={m.members_you_badge_title()}
								>{m.members_you_badge()}</span>
							{/if}
							{#if !member.email_id}
								<span
									class="rounded bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800"
									title={m.members_roster_only_title()}
								>{m.members_roster_only_badge()}</span>
							{/if}
						</div>
						{#if member.email}
						...
						<p class="mt-1 text-xs text-gray-500">
							{m.members_joined_date({ date: new Date(member.joinedAt).toLocaleDateString() })}
						</p>

						<!-- Voices section with inline editing -->
						<div class="mt-3">
							<div class="flex items-center gap-2">
								<span class="text-sm font-medium text-gray-700">{m.members_voices_label()}</span>
								...
								{#if isAdmin && updatingMember !== member.id}
									<div class="relative">
										<button
											...
											title={m.members_add_voice_title()}
										>
											{m.members_add_btn()}
										</button>
						...
						<div class="mt-2">
							<div class="flex items-center gap-2">
								<span class="text-sm font-medium text-gray-700">{m.members_sections_label()}</span>
								...
								{#if isAdmin && updatingMember !== member.id}
									<div class="relative">
										<button
											...
											title={m.members_add_section_title()}
										>
											{m.members_add_btn()}
										</button>
						...
						<div class="mt-3 flex flex-wrap gap-2">
							{#each ASSIGNABLE_ROLES as role}
								...
								<button
									...
									aria-label={m.members_role_aria({ action: hasRole ? m.actions_delete() : m.actions_add(), role, name: member.name })}
								>
...
<p class="mt-6 text-sm text-gray-500">
	{m.members_member_count({ filtered: String(filteredMembers.length), total: String(members.length) })}
</p>
