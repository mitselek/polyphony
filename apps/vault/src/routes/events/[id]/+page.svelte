<script lang="ts">
  ...
  // Local state for program (passed to component)
  let program = $state(untrack(() => [...data.program]));
  
  // Local state for repertoire (passed to component)  
  let repertoire = $state(untrack(() => data.repertoire));
  let availableWorks = $state(untrack(() => data.availableWorks));
